...
``multiprocessing.Process`` that allows fmripost_tedana to enforce
...
def build_workflow(config_file, retval):
    ...
    banner = [f'Running fMRIPost-tedana version {version}']
    notice_path = Path(pkgrf('fmripost_tedana', 'data/NOTICE'))
    if notice_path.exists():
        banner[0] += '\n'
        banner += [f'License NOTICE {"#" * 50}']
        banner += [f'fMRIPost-tedana {version}']
    ...
    # Build main workflow
    init_msg = [
        "Building fMRIPost-tedana's workflow:",
    ...
    if config.execution.datasets:
        init_msg += [f'Searching for datasets: {config.execution.datasets}.']

    build_log.log(25, f'\n{" " * 11}* '.join(init_msg))

    retval['workflow'] = init_fmripost_tedana_wf()

    # Check workflow for missing commands
    missing = check_deps(retval['workflow'])
    if missing:
        build_log.critical(
            'Cannot run fMRIPost-tedana. Missing dependencies:%s',
    ...
    build_log.info(
        'fMRIPost-tedana workflow graph with %d nodes built successfully.',
...
def build_boilerplate(config_file, workflow):
    ...
    if not config.execution.md_only_boilerplate and citation_files['md'].exists():
        ...
        bib_text = Path(pkgrf('fmripost_tedana', 'data/boilerplate.bib')).read_text()
        citation_files['bib'].write_text(
            bib_text.replace(
                'fMRIPost-tedana <version>',
                f'fMRIPost-tedana {config.environment.version}',
            )
        )
        ...
        cmd = [
            ...
            '--citeproc',
            '--metadata',
            'pagetitle="fMRIPost-tedana citation boilerplate"',
