...
def check_latest():
    ...
    cachefile = Path.home() / '.cache' / 'fmripost_tedana' / 'latest'
    try:
    ...
    if latest is None or outdated is True:
        ...
        with suppress(Exception):
            response = requests.get(
                url='https://pypi.org/pypi/fmripost_tedana/json', timeout=1.0
            )
...
def is_flagged():
    ...
    # https://raw.githubusercontent.com/nipreps/fmripost_tedana/main/.versions.json
    flagged = ()
    response = None
    with suppress(Exception):
        response = requests.get(
