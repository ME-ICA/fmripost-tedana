...
#
r"""
A Python module to maintain unique, run-wide *fMRIPost-tedana* settings.

This module implements the memory structures to keep a consistent, singleton config.
Settings are passed across processes via filesystem, and a copy of the settings for
each run and subject is left under
``<fmriprep_dir>/sub-<participant_id>/log/<run_unique_id>/fmripost_tedana.toml``.
Settings are stored using :abbr:`ToML (Tom's Markup Language)`.
The module has a :py:func:`~fmripost_tedana.config.to_filename` function to allow writing out
the settings to hard disk in *ToML* format, which looks like:

.. literalinclude:: ../fmripost_tedana/data/tests/config.toml
   :language: toml
   :name: fmripost_tedana.toml
   :caption: **Example file representation of fMRIPost-tedana settings**.

This config file is used to pass the settings across processes,
using the :py:func:`~fmripost_tedana.config.load` function.
...
.. code-block:: Python
    ...
    config_file = config.execution.work_dir / '.fmripost_tedana.toml'
...
CONFIG_FILENAME = 'fmripost_tedana.toml'
...
if not any(
    ...
    # https://github.com/nipreps/fmripost_tedana/pull/2080#discussion_r409118765
    from fmriprep._warnings import logging
else:
    import logging
...
if os.getenv('IS_DOCKER_8395080871'):
    ...
    if _cgroup.exists() and 'docker' in _cgroup.read_text():
        ...
        _exec_env = 'fmripost_tedana-docker' if _docker_ver else 'docker'
    del _cgroup
...
class environment(_Config):
    ...
    execution environment, nipype and *fMRIPost-tedana* versions, etc.).
...
class execution(_Config):
    ...
    notrack = False
    """Do not collect telemetry information for *fMRIPost-tedana*."""
    track_carbon = False
...
class workflow(_Config):
    ...
    err_on_warn = False
    """Cast tedana warnings to errors."""
    melodic_dim = None
    ...
    ignore = None
    """Ignore particular steps for *fMRIPost-tedana*."""
    cifti_output = None
...
def load(filename, skip=None, init=True):
    ...
    filename : :py:class:`os.PathLike`
        TOML file containing fMRIPost-tedana configuration.
    skip : dict or None
