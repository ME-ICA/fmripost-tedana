...
def run_command(command, env=None):
    ...
    Keep this out of the real fmripost_tedana code so that devs don't need
...
def run_tests(test_regex, test_mark):
    ...
    mounted_code = '/usr/local/miniconda/lib/python3.10/site-packages/fmripost_tedana'
    run_str = 'docker run --rm -ti '
    run_str += f'-v {local_patch}:{mounted_code} '
    run_str += '--entrypoint pytest '
    run_str += 'pennlinc/fmripost_tedana:unstable '
    run_str += (
        f'{mounted_code}/fmripost_tedana '
        f'--data_dir={mounted_code}/fmripost_tedana/tests/test_data '
        f'--output_dir={mounted_code}/fmripost_tedana/tests/pytests/out '
        f'--working_dir={mounted_code}/fmripost_tedana/tests/pytests/work '
    )
    if test_regex:
