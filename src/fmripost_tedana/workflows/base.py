...
#
"""
fMRIPost tedana workflows
^^^^^^^^^^^^^^^^^^^^^^^^

.. autofunction:: init_fmripost_tedana_wf
...
from fmripost_tedana.utils.utils import _get_wf_name, update_dict


def init_fmripost_tedana_wf():
    """Build *fMRIPost-tedana*'s pipeline.

    This workflow organizes the execution of fMRIPost-tedana,
    ...
    Workflow Graph
        .. workflow::
            ...
            from fmripost_tedana.workflows.base import init_fmripost_tedana_wf

            with mock_config():
                wf = init_fmripost_tedana_wf()
    ...
    fmripost_tedana_wf = Workflow(name=f'fmripost_tedana_{ver.major}_{ver.minor}_wf')
    fmripost_tedana_wf.base_dir = config.execution.work_dir
    ...
    for subject_id in config.execution.participant_label:
        ...
        fmripost_tedana_wf.add_nodes([single_subject_wf])
        ...
        config.to_filename(log_dir / 'fmripost_tedana.toml')

    return fmripost_tedana_wf
...
def init_single_subject_wf(subject_id: str):
    ...
    from fmripost_tedana.interfaces.bids import DerivativesDataSink
    from fmripost_tedana.interfaces.reportlets import AboutSummary, SubjectSummary
    from fmripost_tedana.utils.bids import collect_derivatives
...
Many internal operations of *fMRIPost-tedana* use
*Nilearn* {NILEARN_VERSION} [@nilearn, RRID:SCR_001362].
For more details of the pipeline, see [the section corresponding
to workflows in *fMRIPost-tedana*'s documentation]\
(https://fmripost_tedana.readthedocs.io/en/latest/workflows.html \
"FMRIPrep's documentation").


### Copyright Waiver

The above boilerplate text was automatically generated by fMRIPost-tedana
...
def init_single_run_wf(bold_file):
    """Set up a single-run workflow for fMRIPost-tedana."""
    from fmriprep.utils.misc import estimate_bold_mem_usage
    from nipype.interfaces import utility as niu
    from niworkflows.engine.workflows import LiterateWorkflow as Workflow

    from fmripost_tedana.utils.bids import collect_derivatives, extract_entities
    from fmripost_tedana.workflows.tedana import init_denoise_wf, init_ica_tedana_wf
    from fmripost_tedana.workflows.outputs import init_func_fit_reports_wf
    ...
    # Run ICA-tedana
    ica_tedana_wf = init_ica_tedana_wf(bold_file=bold_file, metadata=bold_metadata, mem_gb=mem_gb)
    ica_tedana_wf.inputs.inputnode.confounds = functional_cache['bold_confounds']
    ica_tedana_wf.inputs.inputnode.skip_vols = skip_vols
    ...
    if ('bold_mni152nlin6asym' not in functional_cache) and ('bold_raw' in functional_cache):
        # Resample to MNI152NLin6Asym:res-2, for ICA-tedana classification
        from fmriprep.workflows.bold.apply import init_bold_volumetric_resample_wf
        from fmriprep.workflows.bold.stc import init_bold_stc_wf
        from niworkflows.interfaces.header import ValidateImage
        from templateflow.api import get as get_template

        from fmripost_tedana.interfaces.misc import ApplyTransforms

        workflow.__desc__ += """\
Raw BOLD series were resampled to MNI152NLin6Asym:res-2, for ICA-tedana classification.
"""
        ...
        workflow.__desc__ += """\
Preprocessed BOLD series in MNI152NLin6Asym:res-2 space were collected for ICA-tedana
classification.
"""
        ...
        raise ValueError('No valid BOLD series found for ICA-tedana classification.')

    workflow.connect([
        (mni6_buffer, ica_tedana_wf, [
    ...
    if config.workflow.denoise_method:
        # Now denoise the output-space BOLD data using ICA-tedana
        ...
        workflow.connect([
            ...
            (ica_tedana_wf, denoise_wf, [
                ('outputnode.mixing', 'inputnode.mixing'),
                ('outputnode.tedana_features', 'inputnode.classifications'),
            ]),
        ])  # fmt:skip
